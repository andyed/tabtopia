// Hero image extraction for Tabtopia
...
function extractAndSendHeroImages() {
  ...
  // Send the extracted hero images to the background script. Guard the
  // orphaned-content-script case (extension reloaded while this kept running in
  // an open tab) — chrome.runtime.id is undefined then and the call would throw
    ...
    if (window.location.hostname.includes("wikibooks")) {
      ...
      allWikiImages
        ...
        .forEach(img => {
          ...
          const relativeToDoc = rect.top + window.scrollY;
